...
describe("Bond", function () {
  ...
  describe("Create token", function () {
    ...
    describe("Normal flow", function () {
      ...
      it("should set correct bond parameters", async function () {
        ...
        expect(this.bond.reserveToken).to.equal(
          BABY_TOKEN.bondParams.reserveToken
        );
        expect(await Bond.maxSupply(this.token.target)).to.equal(
          BABY_TOKEN.bondParams.maxSupply
        );
      });

      it("should set the correct createdAt timestamp", async function () {
        expect((await this.creationTx.getBlock()).timestamp).to.equal(
          this.bond.createdAt
        );
      });
    ...
    describe("Validations", function () {
      beforeEach(async function () {
        this.newTokenParams = modifiedValues(BABY_TOKEN.tokenParams, {
          symbol: "BABY2",
        });
      });
        ...
        await expect(
          Bond.createToken(
            ...
            modifiedValues(BABY_TOKEN.bondParams, {
              mintRoyalty: MAX_ROYALTY_RANGE + 1n,
            })
          )
        )
          .to.be.revertedWithCustomError(
      ...
      it("should check if burnRoyalty is less than the max range", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              burnRoyalty: MAX_ROYALTY_RANGE + 1n,
            })
          )
        )
          .to.be.revertedWithCustomError(
      ...
      it("should check if the reserve token implements name()", async function () {
        const r2 = await ethers.deployContract("TestToken", [
          wei(200000000),
          "",
          "TEST",
          18n,
      ...
      it("should check if the length of step ranges are more than max steps", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              stepRanges: [...Array(MAX_STEPS + 2n).keys()].splice(1),
            })
          )
        )
          .to.be.revertedWithCustomError(Bond, "MCV2_Bond__InvalidStepParams")
      ...
      it("should check if any of step ranges has zero value", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              stepRanges: [0, BABY_TOKEN.bondParams.maxSupply],
              stepPrices: [1, 2],
            })
          )
        )
      ...
      it("should check if any of step ranges is less than the previous step", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              stepRanges: [wei(2), wei(1), BABY_TOKEN.bondParams.maxSupply],
              stepPrices: [1, 2, 3],
            })
          )
        )
      ...
      it("should check if any of step prices is less than the previous step", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              stepRanges: [wei(1), wei(2), BABY_TOKEN.bondParams.maxSupply],
              stepPrices: [1, 3, 2],
            })
          )
        )
      ...
      it("should revert if any of the step ranges * prices are lower than the multiFactor", async function () {
        await expect(
          Bond.createToken(
            this.newTokenParams,
            modifiedValues(BABY_TOKEN.bondParams, {
              stepRanges: [1n, BABY_TOKEN.bondParams.maxSupply],
              stepPrices: [1n, 2n],
            })
          )
        )
      ...
      it("should revert if token symbol already exists", async function () {
        await expect(
          Bond.createToken(BABY_TOKEN.tokenParams, BABY_TOKEN.bondParams)
        ).to.be.revertedWithCustomError(
          Bond,
    ...
    describe("Update bond creator", function () {
      beforeEach(async function () {
        await Bond.connect(owner).updateBondCreator(
          this.token.target,
          bob.address
        );
      });
      ...
      it("should revert if the msg.sender is not current creator", async function () {
        await expect(
          Bond.connect(owner).updateBondCreator(this.token.target, bob.address)
      ...
      it("should revert if the token does not exist in the bond", async function () {
        await expect(
          Bond.connect(owner).updateBondCreator(NULL_ADDRESS, bob.address)
        ).to.be.revertedWithCustomError(Bond, "MCV2_Bond__PermissionDenied");
      });

      it("should emit BondCreatorUpdated event", async function () {
        await expect(
          Bond.connect(bob).updateBondCreator(this.token.target, bob.address)
        )
    ...
    describe("Mint", function () {
      describe("Mint once", function () {
        beforeEach(async function () {
          ...
          await BaseToken.connect(alice).approve(
            Bond.target,
        ...
        it("should transfer BASE tokens to the bond", async function () {
          expect(await BaseToken.balanceOf(alice.address)).to.equal(
            this.initialBaseBalance - this.mintTest.reserveRequired
          );
          expect(await BaseToken.balanceOf(Bond.target)).to.equal(
            this.mintTest.reserveRequired
          ); // including royalties until claimed
        });
        ...
        it("should increase the total supply", async function () {
          ...
          expect(await this.token.totalSupply()).to.equal(
            BABY_TOKEN.bondParams.stepRanges[0] + this.tokensToMint
          );
        });

        it("should add claimable balance to the creator", async function () {
          expect(
            await Bond.userTokenRoyaltyBalance(owner.address, BaseToken.target)
          ).to.equal(this.mintTest.creatorCut);
        });

        it("should add claimable balance to the protocol beneficiary", async function () {
          expect(
            await Bond.userTokenRoyaltyBalance(
              PROTOCOL_BENEFICIARY,
              BaseToken.target
            )
          ).to.equal(this.mintTest.protocolCut);
        });

        it("should emit Mint event", async function () {
          await expect(
            Bond.connect(alice).mint(
              this.token.target,
              this.tokensToMint,
              MAX_INT_256,
              alice.address
      ...
      describe("Massive mint & burn through multiple steps", function () {
        ...
        describe("Massiv Mint", function () {
          it("should be at the last price step", async function () {
            expect(await Bond.priceForNextMint(this.token.target)).to.equal(
              BABY_TOKEN.bondParams.stepPrices[7]
            );
          });
          ...
          it("should transfer BASE tokens to the bond", async function () {
            ...
            expect(await BaseToken.balanceOf(Bond.target)).to.equal(
              this.initialBaseBalance
            ); // including royalty until claimed
          });
          ...
          it("should add claimable balance to the creator", async function () {
            ...
            ).to.equal(this.predicted.creatorCut);
          });
          ...
          it("should add claimable balance to the protocol beneficiary", async function () {
            ...
            ).to.equal(this.predicted.protocolCut);
          });
          ...
          describe("Massive Burn", function () {
            beforeEach(async function () {
              this.initial = {
                ...
                bondReserve: (await Bond.tokenBond(this.token.target))
                  .reserveBalance,
              };
              ...
              await Bond.connect(alice).burn(
                this.token.target,
                this.initial.tokenBalance,
                0,
                alice.address
              );
            });
            ...
            it("should transfer BASE tokens to alice", async function () {
              const { total } = calculateRoyalty(
                this.initial.bondReserve,
                BABY_TOKEN.bondParams.burnRoyalty
              );
              const toRefund = this.initial.bondReserve - total;
              expect(await BaseToken.balanceOf(alice.address)).to.equal(
                this.initial.baseBalance + toRefund
              );
            });
            ...
            it("should decrease the total supply", async function () {
              expect(await this.token.totalSupply()).to.equal(
                BABY_TOKEN.bondParams.stepRanges[0]
              ); // except the free minting amount
            });
            ...
            it("should add claimable balance to the creator", async function () {
              const mintRoyalty = calculateRoyalty(
                this.initial.bondReserve,
                BABY_TOKEN.bondParams.mintRoyalty
              );
              const burnRoyalty = calculateRoyalty(
                this.initial.bondReserve,
                BABY_TOKEN.bondParams.burnRoyalty
              );
              ...
              ).to.equal(mintRoyalty.creatorCut + burnRoyalty.creatorCut);
            });
            ...
            it("should add claimable balance to the protocol", async function () {
              ...
              ).to.equal(mintRoyalty.protocolCut + burnRoyalty.protocolCut);
            });
      ...
      describe("Burn", function () {
        ...
        it("should decrease the BABY tokens from Alice", async function () {
          expect(await this.token.balanceOf(alice.address)).to.equal(
            this.initial.tokenBalance - this.tokensToBurn
          );
        });

        it("should transfer correct amount of BASE tokens to Alice", async function () {
          expect(await BaseToken.balanceOf(alice.address)).to.equal(
            this.initial.baseBalance + this.burnTest.reserveToRefund
          );
        });
        ...
        it("should decrease the BASE tokens balance from the bond", async function () {
          ...
          expect(await BaseToken.balanceOf(Bond.target)).to.equal(
            this.initial.bondBalance - this.burnTest.reserveToRefund
          );
        });

        it("should decrease the total supply of BABY token", async function () {
          expect(await this.token.totalSupply()).to.equal(
            this.initial.supply - this.tokensToBurn
          );
        });

        it("should deduct reserveBalance from the bond", async function () {
          const bond = await Bond.tokenBond(this.token.target);
          expect(bond.reserveBalance).to.equal(
            this.initial.bondReserve - this.burnTest.reserveFromBond
          );
        });
        ...
        it("should add claimable balance to the creator", async function () {
          expect(
            await Bond.userTokenRoyaltyBalance(owner.address, BaseToken.target)
          ).to.equal(this.mintTest.creatorCut + this.burnTest.creatorCut);
        });
        ...
        it("should add claimable balance to the protocol beneficiary", async function () {
          ...
          ).to.equal(this.mintTest.protocolCut + this.burnTest.protocolCut);
        });

        it("should emit Burn event", async function () {
          await expect(
            Bond.connect(alice).burn(
              this.token.target,
              this.tokensToBurn,
              0,
              alice.address
    ...
    describe("Other Edge Cases", function () {
      describe("Mint: Edge Cases", function () {
        ...
        it("should revert if the slippage limit exceeded due to a front-run", async function () {
          ...
          await Bond.connect(alice).mint(
            this.token.target,
            BABY_TOKEN.bondParams.stepRanges[1],
            MAX_INT_256,
            alice.address
          );
        ...
        it("should revert if user try to mint more than the available supply", async function () {
          const maxTokensToMint =
            BABY_TOKEN.bondParams.stepRanges[
              BABY_TOKEN.bondParams.stepRanges.length - 1
        ...
        it("should revert if user try to mint more than the balance", async function () {
          ...
          await BaseToken.connect(alice).transfer(
            owner.address,
            this.initialBaseBalance - test.reserveRequired
          );
          await expect(
            Bond.connect(alice).mint(
              this.token.target,
      ...
      describe("Burn: Edge Cases", function () {
        ...
        it("should revert if it did not approve", async function () {
          await expect(
            Bond.connect(alice).burn(this.token.target, 100n, 0, alice.address)
          ).to.be.revertedWithCustomError(
            this.token,
            "ERC20InsufficientAllowance"
          );
        ...
        it("should revert if alice try to burn more than the available balance", async function () {
          await this.token
            .connect(alice)
            .approve(Bond.target, this.tokensToMint + 1n);

          await expect(
            Bond.connect(alice).burn(
              this.token.target,
              this.tokensToMint + 1n,
              0,
              alice.address
            )
          ).to.be.revertedWithCustomError(
        ...
        it("should revert if alice try to burn more than the total supply", async function () {
          ...
          await this.token.transfer(
            alice.address,
            await this.token.balanceOf(owner.address)
          );
        ...
        it("should revert if the caller receives a smaller amount than minRefund", async function () {
          ...
          const { reserveToRefund } = calculateBurn(
            burnAmount,
            BABY_TOKEN.bondParams.stepPrices[1],
            BABY_TOKEN.bondParams.burnRoyalty
          );
        ...
        it("should revert if the slippage limit exceeded due to a front-run", async function () {
          ...
          await this.token
            .connect(owner)
            .approve(Bond.target, BABY_TOKEN.bondParams.stepRanges[0]);
          await Bond.connect(owner).burn(
            this.token.target,
            BABY_TOKEN.bondParams.stepRanges[0],
            0,
            owner.address
  ...
  describe("Edge cases: Rounding errors", function () {
    beforeEach(async function () {
      ...
      await Bond.createToken(
        Object.values(EXTREME_BABY.tokenParams),
        Object.values(EXTREME_BABY.bondParams)
      );
      const Token = await ethers.getContractFactory("MCV2_Token");
      this.token = await Token.attach(await Bond.tokens(0));
    ...
    it("requires exact bond amount even if the royalty is omitted due to flooring", async function () {
      ...
      expect((await Bond.tokenBond(this.token.target)).reserveBalance).to.equal(
        predicted.reserveOnBond
      );
      expect(await BaseToken.balanceOf(alice.address)).to.equal(
        this.initialBaseBalance - predicted.reserveRequired
      );
      expect(await BaseToken.balanceOf(Bond.target)).to.equal(
        predicted.reserveRequired
      );
      expect(
        await Bond.userTokenRoyaltyBalance(
          PROTOCOL_BENEFICIARY,
      ...
      ).to.equal(predicted.protocolCut);
      expect(
        await Bond.userTokenRoyaltyBalance(owner.address, BaseToken.target)
      ).to.equal(predicted.creatorCut);
    });
  ...
  describe("Edge cases: Tax Token", function () {
    beforeEach(async function () {
      ...
      this.TaxToken = await ethers.deployContract("TaxToken", [
        this.initialBaseBalance,
      ]);
      ...
      await Bond.createToken(
        Object.values(TAXED_BABY.tokenParams),
        Object.values(TAXED_BABY.bondParams)
      );
      const Token = await ethers.getContractFactory("MCV2_Token");
      this.token = await Token.attach(await Bond.tokens(0));
    ...
    it("should mint the correct amount after tax", async function () {
      ...
      expect(await this.token.balanceOf(alice.address)).to.equal(wei(10));
      expect(await this.TaxToken.balanceOf(alice.address)).to.equal(
        this.aliceBalance - wei(50)
      );
      expect(await this.TaxToken.balanceOf(Bond.target)).to.equal(wei(45)); // after 10% taxed -> undercollateralized!
      expect(await this.TaxToken.balanceOf(owner.address)).to.equal(
        this.ownerBalance + wei(5)
      ); // 10% tax to the onwer
    });
    ...
    it("could work on small amount", async function () {
      await Bond.connect(alice).mint(
        ...
        MAX_INT_256,
        alice.address
      ); // Bond has 45 TAXT, not 50 (undercollateralized)
      await Bond.connect(alice).burn(
        this.token.target,
        wei(4),
        0,
        alice.address
      ); // 20 TAXT can be refunded

      expect(await this.token.balanceOf(alice.address)).to.equal(wei(6));
      expect(await this.TaxToken.balanceOf(alice.address)).to.equal(
        this.aliceBalance - wei(32)
      ); // -50 + 20 - 2 (tax)
    ...
    it("cannot refund the full collateral on burn", async function () {
      ...
      await expect(
        Bond.connect(alice).burn(this.token.target, wei(10), 0, alice.address)
  ...
  describe("Utility functions", function () {
    beforeEach(async function () {
      this.BaseToken2 = await ethers.deployContract("TestToken", [
        wei(200000000),
        "Test Token",
        "TEST",
        18n,
      ...
      this.tx0 = await Bond.connect(alice).createToken(
        ...Object.values(BABY_TOKEN)
      );
      this.tx1 = await Bond.connect(alice).createToken(
        ...Object.values(BABY_TOKEN2)
      );
      this.tx2 = await Bond.connect(bob).createToken(
        ...Object.values(BABY_TOKEN3)
      );
    ...
    it("should return [0] for ReserveToken = BaseToken", async function () {
      const addresses = await Bond.getTokensByReserveToken(
        BaseToken.target,
        0,
        100
      );
    ...
    it("should return detailed bond information", async function () {
      const steps = BABY_TOKEN.bondParams.stepRanges.map((step, i) => [
        step,
        BABY_TOKEN.bondParams.stepPrices[i],
      ]);
